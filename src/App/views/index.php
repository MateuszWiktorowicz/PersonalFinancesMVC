<?php include $this->resolve("partials/_header.php") ?>

<body class="d-flex flex-column">

    <main>
        <section class="sectionBox">
            <div class="d-flex flex-column align-items-center mt-5 mx-5 p-4 ">
                <div class="d-flex text-center col-12 col-md-6 flex-column gap-3 fw-bold rounded mt-5 p-3 mx-2">
                    <div class="fs-4">Why you should start using our app?</div>
                    <div>1. Increased Financial Awareness</div>
                    <div>2. Real-Time Monitoring and Visibility</div>
                    <div>3. Goal Setting and Planning</div>
                </div>
            </div>
        </section>
    </main>
    <?php include $this->resolve("partials/_footer.php") ?>